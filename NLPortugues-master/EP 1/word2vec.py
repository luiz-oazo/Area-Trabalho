...
def negSamplingLossAndGradient(
    ...
    score = sigmoid(np.dot(outsideWordVec, centerWordVec))
    negScores = sigmoid(-np.dot(negSampleVecs, centerWordVec))
    loss = -np.log(score) - np.sum(np.log(negScores))

    gradCenterVec += (score - 1.0) * outsideWordVec
    for i in range(K):
        gradCenterVec += (1.0 - negScores[i]) * negSampleVecs[i]

    gradOutsideVecs[outsideWordIdx] += (score - 1.0) * centerWordVec
    for i, negSampleIdx in enumerate(negSampleWordIndices):
        gradOutsideVecs[negSampleIdx] += (1.0 - negScores[i]) * centerWordVec
...
def skipgram(
    ...
    for outsideWord in outsideWords:
        ...
        curr_loss, gradCenterVec, gradOutsideVec = word2vecLossAndGradient(
